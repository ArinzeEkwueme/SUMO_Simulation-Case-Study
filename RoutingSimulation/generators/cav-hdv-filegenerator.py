...
def get_options(args=None):
    ...
    optParser.add_argument("-CAV", "--CAV-trip-file", dest="CAVf",
                           default="outputs/trips.trips.CAV.xml", help="CAV demand file name")
    optParser.add_argument("-HDV", "--HDV-trip-file", dest="HDVf",
                           default="outputs/trips.trips.xml", help="HDV demand file name")                     
    ...
    optParser.add_argument("-HDVRePr", "--HDV.rerouting.probability", dest="HDVRePr",
                           default="1.0", help="The probability for a HDV to have a routing device")
    optParser.add_argument("-HDVReDe", "--HDV.rerouting.deterministic", dest="HDVReDe",
                           default="true", help="The devices are set deterministic using a fraction of 1000 (with the defined probability")
    ...
    optParser.add_argument("-CAVRePr", "--CAV.rerouting.probability", dest="CAVRePr",
                           default="0", help="The probability for a CAV to have a routing device")
...
def main(options):
    ...
    src_tree = et.parse('outputs/sumoproject.odtrips.xml')
    dest_tree = et.parse('outputs/trips.trips.CAV.xml')
    src_tree_2 = et.parse('outputs/trips.trips.xml')
    ...
    et.ElementTree(dest_root).write('outputs/trips.trips.CAV.xml')
    et.ElementTree(src_root).write('outputs/trips.trips.xml')
